...
class AdminDonHang
{
        ...
        try {
            $query = "
             SELECT san_phams.id, san_phams.ten_san_pham, san_phams.gia_san_pham, 
                    chi_tiet_don_hangs.so_luong, san_phams.hinh_anh as hinh_anh
    ...
    //     ]);
    // }
    // Inside AdminDonHang model
public function updateDonHang($orderId, $orderStatus, $paymentStatus, $paymentMethod)
{
    try {
        $sql = "UPDATE don_hangs SET 
                trang_thai_id = :orderStatus, 
                thanh_toan_id = :paymentStatus, 
        ...
        return $stmt->rowCount() > 0; // Returns true if any rows were updated
    } catch (Exception $e) {
    ...
    public function updateTrangThaiDonHang($orderId, $trangThaiId)
    {
        $sql = "UPDATE don_hangs SET trang_thai_id = :trangThaiId WHERE id = :orderId";
        $stmt = $this->conn->prepare($sql);
    ...
    {
        try {
            // First, delete the related records in chi_tiet_don_hangs
            $query = "DELETE FROM chi_tiet_don_hangs WHERE don_hang_id = :id";
            $stmt = $this->conn->prepare($query);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            $stmt->execute(); // Execute the deletion of related records
    
            // Then, delete the order from don_hangs
            $query = "DELETE FROM don_hangs WHERE id = :id";
            $stmt = $this->conn->prepare($query);
            $stmt->bindParam(':id', $id, PDO::PARAM_INT);
            
            return $stmt->execute(); // Execute the deletion of the order
        } catch (PDOException $e) {
    ...
    {
        try {
            $query = "INSERT INTO chi_tiet_don_hang (don_hang_id, san_pham_id, so_luong) 
                      VALUES (:don_hang_id, :san_pham_id, :so_luong)";
            $stmt = $this->conn->prepare($query);
